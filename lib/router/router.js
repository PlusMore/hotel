...
Router.route('/', function() {
  ...
  waitOn: function() {
    return [
      // waitOn subscriptions at currentTime = now
      // then 'subscriptions' takes over to make subscription reactive via Session currentTime
      Meteor.subscribe('dashboardWidgetInfo', Session.get('hotelId')),
      Meteor.subscribe('roomsAndActiveStays', Session.get('hotelId'))
    ];
  },
  subscriptions: function() {
    this.subscribe('dashboardWidgetInfo', Session.get('hotelId'), Session.get('currentTime'));
...
Router.route('/guest/check-in', function() {
  ...
  waitOn: function () {
    // waitOn subscription at currentTime = now
    // then 'subscriptions' takes over to make subscription reactive via Session currentTime
    return Meteor.subscribe('roomsAndActiveStays', Session.get('hotelId'));
  },
  subscriptions: function() {
    this.subscribe('roomsAndActiveStays', Session.get('hotelId'), Session.get('currentTime'));
  }
...
// STAYS
Router.route('/stays/current', function() {
  this.render('CurrentStays', {});
}, {
  name: "Stays.Current",
  controller: "HotelRequiredController"
});
